...
const queryModel = async (query, image) => {
  ...
  const history = chatHistory.map(entry => `Q: ${entry.query}\nA: ${entry.response}`).join('\n\n');
  console.log(history)
  console.log(chatHistory)
  const prompt = `Here is the context of past questions asked by the user and the responses provided:\n\n${history}\n\nNow, based on the image provided, find the best solution for the user's query to help them understand the concept better. The query is: ${query}.`;
